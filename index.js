...
const initArgParser = ArgParserClass => {
  ...
  parser.addArgument(["-b", "--baseModelName"], { help: "name of base model" });

  parser.addArgument(["-a", "--associationModelName"], {
    help: "array of name of model of associations",
    nargs: "+"
  });

  parser.addArgument(["-s", "--singular"], {
    help:
      "array of singular of alias names (the alias for association, may be same as associationModelName)",
    nargs: "+"
  });

  parser.addArgument(["-p", "--plural"], {
    help: "array of plural of alias names (if n/a then use _)",
    nargs: "+"
  });

  parser.addArgument(["-t", "--type"], {
    help: "array of either BelongsTo or HasMany or HasOne or BelongsToMany",
    nargs: "+"
  });

  parser.addArgument(["-j", "--joinTable"], {
    help:
      "array of join table names, either JoinTableName or an join table attributes e.g. JoinTableAttributes",
    nargs: "+"
  });
...
const addMixinsBelongsTo = (associationModelName, singular) => {
  const AssocInstance = `I${associationModelName}Instance`;
  const AssocAttributes = `I${associationModelName}Attributes`;
  return `
...
const addMixinsBelongsToMany = (
  ...
  const JoinTableAttributes =
    joinTableName[0] === '"' || joinTableName[0] === "'"
      ? joinTableName
      : `I${joinTableName}Attributes`;
...
const generateInterface = ({
  ...
  for (let i = 0; i < associationModelName.length; i++) {
    if (type[i] === "BelongsTo") {
      ...
      mixinsString += addMixinsHasMany(
        associationModelName[i],
        singular[i],
        plural[i]
      );
    } else if (type[i] === "BelongsToMany") {
      mixinsString += addMixinsBelongsToMany(
        associationModelName[i],
        singular[i],
        plural[i],
        joinTable[i]
  ...
  let interfaceString = `export interface I${baseModelName}Instance extends Sequelize.Instance<I${baseModelName}Attributes>, I${baseModelName}Attributes {
      ${mixinsString}
  };
    `;
